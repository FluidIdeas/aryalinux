...
<code class="literal">APPENDDEF(`confENVDEF',`-DSTARTTLS -DSASL -DLDAPMAP') APPENDDEF(`confLIBS', `-lssl -lcrypto -lsasl2 -lldap -llber -ldb') APPENDDEF(`confINCDIRS', `-I/usr/include/sasl')</code>
EOF
patch -Np1 -i $DIR/sendmail.8.15.2-openssl-1.patch &&

cat >> devtools/Site/site.config.m4 << "EOF"
<code class="literal">define(`confMANGRP',`root') define(`confMANOWN',`root') define(`confSBINGRP',`root') define(`confUBINGRP',`root') define(`confUBINOWN',`root')</code>
EOF
