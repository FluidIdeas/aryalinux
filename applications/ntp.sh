...
<code class="literal"># Asia server 0.asia.pool.ntp.org # Australia server 0.oceania.pool.ntp.org # Europe server 0.europe.pool.ntp.org # North America server 0.north-america.pool.ntp.org # South America server 2.south-america.pool.ntp.org driftfile /var/lib/ntp/ntp.drift pidfile /var/run/ntpd.pid leapfile /etc/ntp.leapseconds</code>
EOF
EOF
chmod a+x /tmp/rootscript.sh
sudo /tmp/rootscript.sh
...
cat > /tmp/rootscript.sh <<"EOF"
cat >> /etc/ntp.conf << "EOF"
<code class="literal"># Security session restrict default limited kod nomodify notrap nopeer noquery restrict -6 default limited kod nomodify notrap nopeer noquery restrict 127.0.0.1 restrict ::1</code>
EOF
EOF
chmod a+x /tmp/rootscript.sh
sudo /tmp/rootscript.sh
